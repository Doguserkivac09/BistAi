...
// ── Uyumluluk alias (Berk'in route'ları için) ──────────────────────

import type { RiskInputs, RiskScore } from '@/types/macro';

/**
 * Berk'in api/risk ve api/sectors route'larında kullandığı fonksiyon.
 * RiskInputs → RiskScore formatına dönüştürür.
 */
export function computeRiskScore(inputs: RiskInputs): RiskScore {
  const statusMap: Record<RiskLevel, RiskScore['status']> = {
    low: 'risk-on',
    medium: 'neutral',
    high: 'risk-off',
    critical: 'risk-off',
  ...
  // Basitleştirilmiş skor hesabı (inputs'tan)
  let score = 50; // base

  if (inputs.vix !== null) {
    if (inputs.vix > 30) score += 20;
    else if (inputs.vix > 20) score += 10;
    else score -= 10;
  }
  if (inputs.yield_curve !== null && inputs.yield_curve < 0) score += 15;
  if (inputs.dollar_index !== null && inputs.dollar_index > 105) score += 10;
  if (inputs.bist_regime === 'bear_trend') score += 15;
  else if (inputs.bist_regime === 'bull_trend') score -= 15;
  ...
  return {
    ...
    components: {
      vix_component: inputs.vix ?? 0,
      yield_component: inputs.yield_10y ?? 0,
      currency_component: inputs.dollar_index ?? 0,
      regime_component: inputs.bist_regime === 'bear_trend' ? 75 : inputs.bist_regime === 'bull_trend' ? 25 : 50,
    },
