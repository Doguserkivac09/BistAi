...
|-------|-------|-----------|--------------|--------------------------|
| Bugün | `/bugun` | `BugunScreen` | /api/smart-signal + /api/watchlist + /api/macro(+history) | **v2 sinyal akışı (2026-07-20):** statik "top-8 en güçlü kurulum" yerine filtre çipli (Tümü/Akıllı para/Teknik/Verdict/Hacim) sinyal akışı — tip `smart_money_entered`/`technical_score≥6`/`status==='STRONG'`/`accumulation|distribution` flag'lerinden türetilir (feed boşsa top-8'e zarif düşüş). Ayrı **Takip listem** kartı eklendi (mobilde sinyal akışının altında, masaüstünde sağ rayda) — `/api/watchlist` sembolleri + smart-signal fiyat/verdict eşlemesi, 5+ ise "Daha fazla göster" |
| Portföyüm | `/portfolyo` | `PortfolyoHubScreen` (Özet=`PortfoyumScreen`) | /api/portfolyo + /api/ohlcv + /api/watchlist + /api/smart-signal + /api/price-alerts | **v2 liquid-glass** (2026-07-06): değer kartı (GERÇEK portföy değeri eğrisi = OHLCV kapanış toplamı + günlük değişim + K/Z/Getiri/Maliyet) + sektör donut'u (SVG) + AI notu; ekle/düzenle/sil + CSV + hedef-fiyat KORUNDU. **✅ Portföyüm hub (2026-07-20):** `design_handoff_portfoyum_hub` — mevcut pozisyon/K-Z ekranı DEĞİŞMEDEN "Özet" sekmesi oldu, 3 yeni sekme eklendi: **Takip Listem** (`/api/watchlist` + `/api/smart-signal` fiyat/verdict eşlemesi + `/api/ohlcv` sparkline, sağ ray özet+verdict dağılımı), **Alarmlar** (`/api/price-alerts` CRUD — yalnızca FİYAT alarmı var, toggle endpoint'i yok, handoff'un 4-türlü+toggle'lı listesi yerine mevcut gerçek şema dürüstçe yansıtıldı), **Sinyal Takip** (Bugün ekranındaki sinyal-akışı türetimiyle AYNI mantık — `/api/smart-signal` flags/technical_score/status'tan Tümü/Akıllı para/Teknik/Verdict/Hacim). |
