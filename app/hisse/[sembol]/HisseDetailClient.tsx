...
import { AccordionSignalRow, SIGNAL_VADE } from './components/signal-row';
import { TeknikGostergelerOzeti, TrendOzeti } from './components/teknik-gostergeler';
...
function calcRSI14(closes: number[]): number | null {
  ...
  g /= 14; l /= 14;
  for (let i = 15; i < closes.length; i++) { const d = closes[i]! - closes[i - 1]!; g = (g * 13 + Math.max(0, d)) / 14; l = (l * 13 + Math.max(0, -d)) / 14; }
...
function calcRelVol5(candles: { volume: number }[]): number | null {
  ...
  return parseFloat((candles.slice(-5).reduce((s, c) => s + c.volume, 0) / 5 / avg20).toFixed(2));
}
function computeADV(candles: { close: number; volume: number }[], n = 10): number {
  const s = candles.slice(-n);
  return s.length ? s.reduce((a, c) => a + c.close * c.volume, 0) / s.length : 0;
}
