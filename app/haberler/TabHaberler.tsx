...
function detectCategory(baslik: string): { label: string; cls: string } | null {
  ...
  for (const { keywords, label, cls } of CATEGORY_MAP) {
    if (keywords.some((kw) => lower.includes(kw))) return { label, cls };
  }
  return null;
}
  ...
  useEffect(() => {
    const timer = setInterval(async () => {
      try {
        ...
        const data = await res.json();
        const fresh: HaberItem[] = data.haberler ?? [];
        const newOnes = fresh.filter((h) => !haberlerRef.current.some((old) => old.link === h.link));
        if (newOnes.length > 0) { setPendingCount(newOnes.length); setPendingData(fresh); }
      } catch { /* ignore */ }
    }, 5 * 60 * 1000);
    ...
    <div>
      ...
      {loading && (
        <div className="space-y-4">
          {[1, 2, 3, 4, 5].map((i) => <div key={i} className="h-24 animate-pulse rounded-xl bg-surface" />)}
        </div>
      )}
          ...
          {filteredHaberler.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <p className="text-text-secondary">
                {haberler.length === 0 ? 'Şu an haber yüklenemiyor.' : 'Bu kriterlere uyan haber bulunamadı.'}
              </p>
              {haberler.length === 0 && (
                <button
                  ...
                  {onceCikanlar.map((haber, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <a href={haber.link} target="_blank" rel="noopener noreferrer" className="flex-1">
                ...
                {digerler.map((haber, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <a href={haber.link} target="_blank" rel="noopener noreferrer" className="flex-1 min-w-0">
