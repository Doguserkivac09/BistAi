...
const ONEM_LABEL: Record<Onem, string> = {
  yuksek: 'Yüksek',
  orta:   'Orta',
  dusuk:  'Düşük',
};
      ...
      {[1, 2, 3].map((i) => (
        <span
          ...
          className={cn(
            ...
            i <= filled ? ONEM_COLOR[onem].replace('text-', 'bg-') : 'bg-white/10'
          )}
        />
      ))}
    </span>
...
function ValueChange({ onceki, gerceklesen, beklenti }: { onceki?: string; gerceklesen?: string; beklenti?: string }) {
  ...
  return (
    <div className="flex items-center gap-3 text-[11px] mt-1.5 flex-wrap">
      {onceki && (
        <span className="text-text-secondary/60">Önceki: <span className="text-text-secondary">{onceki}</span></span>
      )}
      {beklenti && !gerceklesen && (
        <span className="text-text-secondary/60">Beklenti: <span className="text-primary/80">{beklenti}</span></span>
      )}
      {gerceklesen && (
    ...
    <div className="container mx-auto max-w-3xl px-4 py-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-text-secondary/60 mt-1">
          TCMB, Fed, ECB faiz kararları · TÜFE, CPI, GSYİH ve diğer kritik veriler
        </p>
      </div>
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-start gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            {ev.aciklama && (
                              <p className="text-[11px] text-text-secondary/50 mt-0.5">{ev.aciklama}</p>
                            )}
