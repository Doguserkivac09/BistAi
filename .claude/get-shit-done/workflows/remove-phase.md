...
```bash
INIT=$(node "C:/Users/MspDo/PROJELERİMİZ/BISTAI/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${target}")
...
```bash
RESULT=$(node "C:/Users/MspDo/PROJELERİMİZ/BISTAI/.claude/get-shit-done/bin/gsd-tools.cjs" phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node "C:/Users/MspDo/PROJELERİMİZ/BISTAI/.claude/get-shit-done/bin/gsd-tools.cjs" phase remove "${target}" --force)
```
...
```bash
node "C:/Users/MspDo/PROJELERİMİZ/BISTAI/.claude/get-shit-done/bin/gsd-tools.cjs" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
